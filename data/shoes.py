...
class ShoesDataset(AbstractBaseShoesDataset):
    ...
    def __getitem__(self, idx):
        ...
        # for noise test
        p = 0
        if bool_based_on_probability(p):
            noise_methods = [0, 1, 2]
            noise_method = random.sample(noise_methods, 1)[0]
            if noise_method == 0:
                modifier_o = random_deletion(modifier, 0.5)
            elif noise_method == 1:
                modifier_o = random_swap(modifier, len(modifier)//2+1)
            else:
                modifier_o = random_insertion(modifier, len(modifier)//2+1)
            if modifier_o is not None and type(modifier_o) is str:
                with open('modifiers.txt', 'a') as f:
                    print(modifier, modifier_o, file=f)
                modifier = modifier_o
...
class ShoesTestDataset(AbstractBaseShoesDataset, AbstractBaseTestDataset):
    """
    ...
    def sample_img_for_visualizing(self, gt):
        #img_path = _create_img_path_from_id(os.path.join(self.img_root_path,self.clothing_type), gt)
        return None
